...
//--------------------------------------------------------
public function getReviewDoctorqueue(Request $request)
{ 
    ...
    if(!empty($result)){
        ...
        if($result['call_id'] == $review_id){
            ...
            <tr><td>'.$review_id.'</td><td>'.$doctor_name.'</td><td>'.$room_no.'</td><td>'.$department_name.'</td><td>'.$last_seen_date.'</td><td>'.'<button class="doct" onclick="queue_doctor('.$doctor.')">Generate Token</button>'.'</td></tr>
            </table>' ;
...
public function getReviewDepartmentqueue(Request $request)
{ 
    $review_id = $request->call_id;
    if(empty($review_id)){
        return '';
    ...
    if(!empty($result)){
        ...
        if($result['call_id'] == $review_id){
           ...
           <input class="pmobile_'.$department.'" name="pmobile" value="'.$pmobile.'" type="hidden"  />
            ...
            <tr><th colspan="6">Your Record Details</th></tr>
            <tr><th>Review ID.</th> <th>Doctor Name</th> <th>Room No.</th> <th>Department</th> <th>Last Seen Date</th><th>&nbsp;</th></tr>
            <tr><td>'.$review_id.'</td><td>'.$doctor_name.'</td><td>'.$room_no.'</td><td>'.$department_name.'</td><td>'.$last_seen_date.'</td><td>'.'<button class="dept" onclick="queue_dept('.$department.')">Generate Token</button>'.'</td></tr>
            </table>' ;
